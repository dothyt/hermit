...
struct page *vm_normal_page_silent(struct vm_area_struct *vma, unsigned long addr,
			    pte_t pte)
{
	unsigned long pfn = pte_pfn(pte);
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (likely(pte_same(*vmf->pte, vmf->orig_pte))) {
		...
		// page_add_vaddr(vmf->pte, new_page, vma, vmf->address);
...
/** [Hermit]
 * hermit_swapin_bypass_swapcache - swap in demand page when its swapcount == 1
 ...
 */
static inline vm_fault_t hermit_swapin_bypass_swapcache(struct page **pagep,
							struct vm_fault *vmf,
							swp_entry_t entry,
							int *cpu,
							int *adc_pf_bits,
							uint64_t pf_breakdown[])
{
	...
	bool spec_io = hmt_enable_spec_io();
	void *shadow = NULL;

	// [Hermit] both sync/async prefetch needs the pref_req
	...
	*pagep = alloc_page_vma(GFP_HIGHUSER_MOVABLE, vma, vmf->address);
	page = *pagep;
	// adc_pf_breakdown_end(pf_breakdown, ADC_ALLOC_PAGE, pf_cycles_end());
	if (!page) {
	...
	if (hermit_mem_cgroup_swapin_charge_page(page, vma->vm_mm, GFP_KERNEL,
	...
	if (shadow) {
		unsigned long rft_dist =
			hermit_workingset_refault(page, shadow);
		if (rft_dist) // [Hermit] update refault distance
			hmt_update_rft_dist(page, rft_dist);
	}
				  ...
				  uint64_t pf_breakdown[])
{
	...
	if (!page) {
		...
		if (hmt_ctl_flag(HMT_BPS_SCACHE) && __swap_count(entry) == 1) {
			/* skip swapcache */
			vm_fault_t func_ret = 0;
			adc_pf_breakdown_stt(pf_breakdown, ADC_PAGE_IO,
					     pf_cycles_start());
			func_ret = hermit_swapin_bypass_swapcache(
			...
			page = hermit_swapin_readahead(entry,
	...
	// at this point, page should be locked
	if (!PageLocked(page))
		pr_err("page not locked! %s:%d\n", __func__, __LINE__);
	...
	page = hermit_ksm_might_need_to_copy(page, vma, vmf->address, cpu);
	if (unlikely(!page)) {
		ret = VM_FAULT_OOM;
		page = swapcache;
	...
	adc_pf_breakdown_end(pf_breakdown, ADC_UPD_METADATA, pf_cycles_end());

	if (cpu != -1 && !(page != swapcache && swapcache)) {
		hermit_poll_read(cpu, page, false, pf_breakdown);
		// lru_cache_add(page);
		cpu = -1;
		adc_pf_breakdown_end(pf_breakdown, ADC_PAGE_IO,
				     pf_cycles_end());
	...
	if (vmf->flags & FAULT_FLAG_WRITE) {
		...
		if (ret & VM_FAULT_ERROR)
			ret &= VM_FAULT_ERROR;
		// page_add_vaddr(vmf->pte, page, vma, vmf->address);
		goto out;
	}
...
unlock:
	pte_unmap_unlock(vmf->pte, vmf->ptl);
	// if (page) // add vpage only after valid rmap established
	// 	page_add_vaddr(vmf->pte, page, vma, vmf->address);
out:
	if (si)
		put_swap_device(si);
...
out_page:
	...
	unlock_page(page);
out_release:
	if (cpu != -1) { // [Hermit] shouldn't happen
		pr_err("%s:%d [Hermit] error: unresolved conflict page\n",
		       __func__, __LINE__);
		hermit_poll_read(cpu, page, false, pf_breakdown);
		cpu = -1;
		adc_pf_breakdown_end(pf_breakdown, ADC_PAGE_IO,
...
inline vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	return do_swap_page_profiling(vmf, NULL, NULL);
}
 ...
 */
static vm_fault_t handle_pte_fault_profiling(struct vm_fault *vmf,
					     int *adc_pf_bits,
					     uint64_t pf_breakdown[])
{
	pte_t entry;
...
{
	return handle_pte_fault_profiling(vmf, NULL, NULL);
}
 ...
 * return value.  See filemap_fault() and __lock_page_or_retry().
 */
static vm_fault_t __handle_mm_fault_profiling(struct vm_area_struct *vma,
...
static inline vm_fault_t __handle_mm_fault(struct vm_area_struct *vma,
					      unsigned long address,
					      unsigned int flags)
{
	return __handle_mm_fault_profiling(vma, address, flags, NULL, NULL);
}
 ...
 * return value.  See filemap_fault() and __lock_page_or_retry().
 */
inline vm_fault_t handle_mm_fault(struct vm_area_struct *vma, unsigned long address,
			   unsigned int flags, struct pt_regs *regs)
{
	return handle_mm_fault_profiling(vma, address, flags, regs, NULL, NULL);
}
vm_fault_t handle_mm_fault_profiling(struct vm_area_struct *vma,
				     unsigned long address, unsigned int flags,
				     ...
				     uint64_t pf_breakdown[])
{
	...
	if (unlikely(is_vm_hugetlb_page(vma))) {
		...
		ret = __handle_mm_fault_profiling(vma, address, flags,
						  adc_pf_bits, pf_breakdown);
	}
