 ...
 */
int add_to_swap_profiling(struct page *page, uint64_t pf_breakdown[])
{
	swp_entry_t entry;
	int err;
...
{
	return add_to_swap_profiling(page, NULL);
}
					   ...
					   uint64_t pf_breakdown[])
{
	...
	// [Hermit]
	bool hmt_spec_io = hmt_enable_spec_io();
...
static inline struct page *
swap_vma_readahead(swp_entry_t fentry, gfp_t gfp_mask, struct vm_fault *vmf)
{
				     ...
				     uint64_t pf_breakdown[])
{
	...
	return hermit_swap_vma_readahead(entry, gfp_mask, vmf, adc_pf_bits,
					 pf_breakdown);
}
			...
			int *adc_pf_bits, uint64_t pf_breakdown[])
{
	...
	// blk_start_plug(&plug);
	for (i = pref_req->stt, pte = ra_info->ptes; i < ra_info->nr_pte;
	     i++, pte++) {
